...
# LinuxMint
mint_ver="$(curl -sL https://www.linuxmint.com/torrents/ | grep -oP '(?<=href=")linuxmint-[0-9]+(\.?[0-9]*)*-cinnamon-64bit.iso.torrent(?=">)' | sort -Vr | head -n 1)"
...
# Kali Linux
kali_ver="$(curl -sL https://cdimage.kali.org/kali-images/current/ | grep -oP '(?<=href=")kali-linux-[0-9]+(\.?[0-9]*)*-live-amd64\.iso\.torrent(?=")' | sort -rV | head -n 1)"
kali_url="https://cdimage.kali.org/kali-images/current/${kali_ver}"

# Kali everything
kali_e_ver="$(curl -sL https://cdimage.kali.org/kali-images/current/ | grep -oP '(?<=href=")kali-linux-[0-9]+(\.?[0-9]*)*-live-everything-amd64\.iso\.torrent(?=")' | sort -rV | head -n 1)"
